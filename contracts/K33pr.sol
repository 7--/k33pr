...
interface IK33prNetworkHelper {
    function getQuoteLimit(uint gasUsed) external view returns (uint);
}

contract K33prNetwork is ReentrancyGuard {
    using SafeMath for uint;
    using SafeERC20 for IERC20;

    /// @notice K33prNetwork Helper to set max prices for the ecosystem
    IK33prNetworkHelper public KPRH;
    ...
    function applyCreditToJob(address provider, address liquidity, address job) external {
        ...
        uint _liquidity = K33prNetworkLibrary.getReserve(liquidity, address(this));
    ...
    */
     ...
     */
    function setK33prNetworkHelper(IK33prNetworkHelper _kprh) external {
        require(msg.sender == governance, "setK33prNetworkHelper: !gov");
        KPRH = _kprh;
    }
